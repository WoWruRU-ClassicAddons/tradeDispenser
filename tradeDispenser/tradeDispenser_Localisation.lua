...
if (not tradeDispenser_Version) then 	tradeDispenser_Version = "0.95" 	end
...
function tradeDispenser_GetRussian()
	...
	tD_Loc = {
		...
		["about"]		= {	-- open end. could have more than 5 lines
			[0] = "О "..tradeDispenser_ProgName,
			[1] = "  Версия: "..tradeDispenser_Version,
			[2] = "  Автор:  Kaboom  @ Arthas (EU)",
			[3] = "  Ядро:    Shag    @ Arthas (EU)",
			[4] = "  Больше:    Пожалуйста прочтите ReadMe",
		},
		["help"]		= {	-- open end. could have more than 9 lines
			[0] = "Префикс:",
			[1] = "  /td или /tradeDispenser       Показывает Помощь",
			[2] = "Опции (Используте: Префикс + Опции)",
			[3] = "  config         Переключает ConfigFrame",
			[4] = "  toggle         Активирует/Деактивирует "..tradeDispenser_ProgName,
			[5] = "  broadcast   Передает обмен",
			[6] = "  OSD           Переключает ЭМ (Горячие кнопки)",
			[7]	= "  verbose X   Показывает информацию отладки. (Уровень X: 0=выкл, 3=ВСЕ)",
			[8] = "  resetpos      Восстанавливает все окна tradeDispenser",
			[9] = "  about          Показыват больше информации о "..tradeDispenser_ProgName,
		},
		["whisper"]		= {		-- default messages - could be edited ingame
			[1] = {
				["short"]	= "что-то не так",
			...
			[2]	= {
				["short"]	= "Предметов не осталось (спам)",
				["default"]	= tradeDispenser_ProgName..": У меня закончились предметы, пожалуйста не надо предлагать мне обмен.",
			},
			[3] = {
				["short"]	= "Стак не полный",
				["default"]	= tradeDispenser_ProgName..": Извини, что последний стак не полный. К сожалению в инвентаре у меня больше нет.",
			},
			[4]	= {
		...
		["Broadcast"] 				= {
			...
			[2]						= "Измените эту строку, чтобы создать еще одно случайное сообщение",
		...
		["buttons"]			= {
			["tradecontrol"] 		= "Управление обменом",
			["settings"]			= "Настройки",
			["enabled"]				= "Активирован",
		...
		["settings"]		= {
			...
			["osdtooltip"]	= {	-- open end. could have more than 3 lines
				...
				[1] = "Активировать, чтобы сделать видимым",
			},
			["broadcasttooltip"]	= {
			...
			["Hint"]	= {	-- open end. could have more than 4 lines
				...
				[3] = "Все классы + Воин + Милишники",
				[4] = "Кроме того лечащие классы не получат",
				[5] = "предметы из профиля 'Ипользующие ману'",
			},
			["Timelimit"] = "Срок по времени",
			["TimerTooltip"] = {
				[0] = "Эта функция запустит таймер,",
				[1] = "который по истечению времени отменит",
				[2] = "обмен. 5 секунд до этого твой клиент",
				[3] = "получит сообщение.",
				[4] = "Внимание: Это произойдет только тогда,",
				[5] = "когда ТЫ уже принял обмен."
			},			
		},
		["OSD"]				= {
			["OSDtitle"]	= "ЭМ",
			["notenabled"]	= tradeDispenser_ProgName.." не активен",
			["broadcasttip"]	= {
				["title"] 		= "Передача обмена",
				["channel"]		= "Канал сообщения: ",	-- here follows the localised channel-name
				["left"]		= "Левый клик: Сообщить сразу",
				["right"]		= "Правый клик: Автоматически сообщать",
			},
			["actbutton"] 	= {
				["activate"]	= "Активировать "..tradeDispenser_ProgName,
				["deactivate"]	= "Деактивировать "..tradeDispenser_ProgName,
				["left"]	= "Левый клик: Переключатель "..tradeDispenser_ProgName,
			...
			["configbtn"]	= {
				...
				["right"]	= "Правый клик: открыть полный центр настроек",
			},
		},	
		["control"]			= {
			...
			["notinraid"]	= {	-- open end. could have more than 5 lines
				...
				[3]			= "Эта функция работает только тогда, когда ты",
				[4]			= "состоишь в рейде или группе",
			},
			...
			["accguild"]	= {	-- open end. could have more than 3 lines
				...
				[1]			= "Принимать их обмен даже тогда, когда",
				[2]			= "они не состоят в рейде/группе",
			},
			["FreeCheckBox"]= "Бесплатно для рейда/гильдии",
			["Free4Guild"]	= {
			...
			["level"]		= "Минимальный: ",
			["register"]	= "Регистрировать клиента",
			["maxtrades"]	= "Макс. количество обмена на клиент: ",
		},
		["Banlist"]			= {
		...
		["KeyBindings"]		= {
			["header"]		= tradeDispenser_ProgName,
			[1]				= "Переключатель для tradeDispenser",
			[2]				= "Показать окно конфига",
			[3]				= "Показать ЭМ",
			[4]				= "Мгноменное вещание",
		},		
